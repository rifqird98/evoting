...
<body>
    <div class="container-fluid">
        ...
        <div class="card border border-0 shadow shadow-lg bg-success">
            <div class="card-body">
                <p class="fs-5 fw-bold text-center text-light">Berikut Adalah Calon Tetap Pimpinan Ranting Muhammadiyah
                    Sepanjang :
                </p>
                <p class="text-center fw-bold text-light mb-3">Pilih Calon Pimpinan 9 orang tidak boleh lebih atau
                    kurang</p>
                <hr />
...
<script>
    ...
    function toggleActive(itemId) {
        ...
        if (index === -1) {
            ...
            if (clickedIds.length >= 9) {
                Swal.fire({
                    title: 'Konfirmasi',
                    text: 'Anda Sudah Memilih 9 Calon Pimpinan, Apakah Anda Akan Menyimpan Pilihan Anda ?',
                    icon: 'question',
    ...
    function getDataCalon() {
        ...
        $.ajax({
            url: "/getDaftarCalon?pimpinan=PRA",
            method: "GET",
            dataType: "JSON",
    ...
    function getDataPemilih() {
        ...
        tbl.html('');
        $.ajax({
            url: "/getPemilihPra?namaPerangkat=" + namaPerangkat,
            method: "GET",
            dataType: "JSON",
            success: function(data, textStatus, jqXHR) {
                if (data.nama) {
                    dataPemilih = data
                    var content = ""
                    content = ` <h3 class="fw-bold">Selamat Datang Ibu, <span class="text-light">${data.nama}</span></h3>
